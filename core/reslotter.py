...
def add_missing_files(reslotted_files, fighter_name, target_alt, is_new_slot=False):
    ...
    # Add camera entry in the correct structure (ONLY in the cXX/camera structure)
    ...
    # Remove any old camera entry that uses a different structure
    ...
    # Extended list of custom file extensions not part of vanilla Smash
    custom_extensions = [
    ...
    # Scan for custom files in the mod folder
    for file in fighter_files:
        # Detect transplanted effects
        ...
        # Check if the file is in the target_alt folder
        if f"/{target_alt}/" in file or file.endswith(f"/{target_alt}"):
            # Handle camera files specially
            if file.startswith(f"camera/fighter/{fighter_name}/{target_alt}/"):
                # Only include .nuanmb for camera, exclude .kime
            ...
            # Check if it is a custom file by extension
            if file_ext in custom_extensions:
                is_custom = True

            # Or if not in known vanilla Smash files
            if file not in known_files:
                is_custom = True

            # Or if it is a custom texture/model file
            if any(marker in file.lower() for marker in ['body', 'face', 'hair', 'eye', 'brs_', 'bust_', 'hand_']):
                is_custom = True

            # If custom, add it to the list
            if is_custom:
                custom_files.append(file)

    # Add custom files to the config
    ...
    # Add slot-specific effect files to the config
    ...
    # Add camera files to the camera folder (ONLY to cXX/camera, not camera/cXX)
    # Only include .nuanmb files, not .kime
    ...
    # Add transplanted effects to fighter/{fighter_name}/cmn
    ...
    # Process normal reslotted files
    for file in reslotted_files:
        # Do not add camera files to the main new-dir-files
        if file.startswith(f"camera/fighter/{fighter_name}/{target_alt}/"):
            continue

        # Do not add transplanted effects to the main new-dir-files
        if f"effect/fighter/{fighter_name}/transplant/" in file:
            continue
            
        # Effect files from the mod always go into config
        # (needed for slot-based effect plugins like Added-Slot Effects)
        is_effect = "effect" in file
        if is_effect:
            if file not in resulting_config["new-dir-files"][new_dir_info]:
...
def IsShareableSound(sound_file):
    # Returns True for all sound files to ensure they are included in config.json
    return True

def addSharedFiles(src_files, source_color, target_color, share_slot):
    used_files = []
    
    # Extensions that are normally not shared, but handled specially (audio files are included)
    ...
    for index in src_files:
        ...
        # Basic filter for textures (audio files are not filtered)
        ...
        if "motion/" in file_path or "camera/" in file_path:
            ...
            # Always add sound files to share-to-added
            share_to = "share-to-added"
...
def main(mod_directory, hashes_file, fighter_name, current_alt, target_alt, share_slot, out_dir):
    ...
    # Remove the source slot entry — it belongs to the source mod, not the reslotted output
    source_key = f"fighter/{fighter_name}/{current_alt}"
    if source_key in resulting_config["new-dir-files"]:
        del resulting_config["new-dir-files"][source_key]

    # Remove empty new-dir-files entries (e.g. unused camera/cmn keys)
    resulting_config["new-dir-files"] = {
        k: v for k, v in resulting_config["new-dir-files"].items() if v
    }
